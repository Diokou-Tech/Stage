        ...
        <div class="col-5">
            ...
            <ul class="list-group-flush">
                <li class="list-group-item d-flex justify-content-around align-items-center">{{ $voeu1->matricule }} {{ $voeu1->nom }} {{ $voeu1->prenom }} 
                    @if($voeu1->stages->count() < 3 )
                    <span class="bg-success text-white p-2 rounded"> {{ $voeu1->stages->count() }} encadré(s)</span>
                    @elseif($voeu1->stages->count() <= 4)
                    <span class="bg-warning text-white p-2 rounded"> {{ $voeu1->stages->count() }} encadré(s)</span>
                    @else
                ...
                <li class="list-group-item d-flex justify-content-around align-items-center">{{ $voeu2->matricule }} {{ $voeu2->nom }} {{ $voeu2->prenom }} 
                    ...
                    @elseif($voeu2->stages->count() <= 4)
                    <span class="bg-warning text-white p-2 rounded"> {{ $voeu2->stages->count() }} encadré(s)</span>
                    @else
                ...
                <li class="list-group-item d-flex justify-content-around align-items-center">{{ $voeu3->matricule }} {{ $voeu3->nom }} {{ $voeu3->prenom }} 
                    ...
                    @elseif($voeu3->stages->count() <= 4)
                    <span class="bg-warning text-white p-2 rounded"> {{ $voeu3->stages->count() }} encadré(s)</span>
                    @else
                    <span class="bg-danger text-white p-2 rounded"> {{  $voeu2->stages->count()}} encadré(s)</span>
                    @endif  
                </li>
